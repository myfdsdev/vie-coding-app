...
const Body = z.object({
  /** The `check` id from the collect event this report answers. */
  check: z.number().int().min(0).max(200),
...
export async function POST(req: Request, { params }: { params: { turnId: string } }) {
  ...
  if (!parsed.success) return Response.json({ error: 'Expected { check, events }' }, { status: 400 });
  ...
  deliverReport(params.turnId, parsed.data.check, events);
